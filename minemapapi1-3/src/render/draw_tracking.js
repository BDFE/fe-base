 ...
 * @private
 */
module.exports = function drawTracking(painter, sourceCache, layer, coords) {
    ...
    if (layer.paint['tracking-width'] <= 0) return;
    //hack me
    segmentGroup = layer.paint['tracking-seg-group'] || 20;

    if (layer.paint['tracking-type'] === 'history') {
        if (!this.histCount) {
        ...
        if ((this.histEnd - this.histStart) >= 1000.0 / layer.paint['tracking-speed']) {
            this.histCount++;

            this.histCount %= layer.paint['tracking-seg-count'];
        ...
        if ((this.end - this.start) >= 999 / layer.paint['tracking-speed']) {
    ...
    const programId =
        layer.paint['tracking-dasharray'] ? 'lineSDF' :
        layer.paint['tracking-pattern'] ? 'linePattern' : 'tracking';
    ...
    for (const coord of coords) {
        ...
        drawTrackingTile(program, painter, tile, bucket.buffers, layer, coord, layerData, programChanged, tileRatioChanged,layer.paint['tracking-type'] === 'history' ? this.histCount : this.count);
        prevTileZoom = tile.coord.z;
        firstTile = false;
    }
};

function drawTrackingTile(program, painter, tile, buffers, layer, coord, layerData, programChanged, tileRatioChanged, link_count) {
    const gl = painter.gl;
    const dasharray = layer.paint['tracking-dasharray'];
    const image = layer.paint['tracking-pattern'];
    ...
    if (programChanged || tileRatioChanged) {
        ...
        if (dasharray) {
            posA = painter.lineAtlas.getDash(dasharray.from, layer.layout['tracking-cap'] === 'round');
            posB = painter.lineAtlas.getDash(dasharray.to, layer.layout['tracking-cap'] === 'round');
    ...
    if (programChanged) {
        ...
        gl.uniform1f(program.u_width, layer.paint['tracking-width']);
    }

    painter.enableTileClippingMask(coord);

    const posMatrix = painter.translatePosMatrix(coord.posMatrix, tile, layer.paint['tracking-translate'], layer.paint['tracking-translate-anchor']);
