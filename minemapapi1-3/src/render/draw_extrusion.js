...
function draw(painter, source, layer, coords) {
    if (layer.paint['extrusion-opacity'] === 0) return;
    const gl = painter.gl;
...
ExtrusionTexture.prototype.renderToMap = function() {
    ...
    gl.uniform1f(program.u_opacity, this.layer.paint['extrusion-opacity']);
...
function drawExtrusion(painter, source, layer, coord) {
    ...
    const image = layer.paint['extrusion-pattern'];

    const layerData = buffers.layerData[layer.id];
    const programConfiguration = layerData.programConfiguration;
    const program = painter.useProgram(image ? 'extrusionPattern' : 'extrusion', programConfiguration);
