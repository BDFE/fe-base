...
module.exports = drawSymtracking;
//hack me
var _count = 0;
var offset = 10;

function drawSymtracking(painter, sourceCache, layer, coords) {
    ...
    const fps = layer.layout['symtracking-fps'];
    const timeSeg = layer.layout['symtracking-time-segment'];

    if ((this.end - this.start) >= (1000 / fps)) {
        this.count += 1;

        this.count %= (fps * (timeSeg + offset));

        if (this.count === fps * timeSeg) {
...
//hack me
function symtrackingDynDeal(gl, program) {
    gl.uniform1f(program.u_seq, _count);
}
...
function drawLayerSymbols(painter, sourceCache, layer, coords, isText, translate, translateAnchor,
    ...
    for (const coord of coords) {
        ...
        if (!program || bucket.fontstack !== prevFontstack) {
            program = painter.useProgram(isSDF ? 'symtrackingSDF' : 'symtrackingIcon', programConfiguration);
