...
var curEleObj = {
    ...
    speedExt3:5,
    speedExt4:6,
    getCalculatedSpeed: function (factor) {
        return parseInt(this['speed' + factor] / factor/this['speedExt'+factor]);
    }
}

module.exports = function drawCubicline(painter, sourceCache, layer, coords) {
    ...
    if (layer.paint['cubicline-width'] <= 0) return;

    //这里处理关于速度的问题
    var speedFactor = parseInt(layer.paint['cubicline-speed-factor']);

    curEleObj["speed" + speedFactor]++;

    curEleObj.speed1 %= 20;
    curEleObj.speed2 %= 80;
    curEleObj.speed3 %= 150;
    curEleObj.speed4 %= 240;

    const programId =
        layer.paint['cubicline-dasharray'] ? 'lineSDF' :
        layer.paint['cubicline-pattern'] ? 'linePattern' : 'cubicline';
    ...
    for (const coord of coords) {
        ...
        drawCubiclineTile(program, painter, tile, bucket.buffers, layer, coord, layerData, programChanged, tileRatioChanged);
        prevTileZoom = tile.coord.z;
        firstTile = false;
    }
};

function drawCubiclineTile(program, painter, tile, buffers, layer, coord, layerData, programChanged, tileRatioChanged) {
    const gl = painter.gl;
    const dasharray = layer.paint['cubicline-dasharray'];
    const image = layer.paint['cubicline-pattern'];
    ...
    if (programChanged || tileRatioChanged) {
        ...
        if (dasharray) {
            posA = painter.lineAtlas.getDash(dasharray.from, layer.layout['cubicline-cap'] === 'round');
            posB = painter.lineAtlas.getDash(dasharray.to, layer.layout['cubicline-cap'] === 'round');
    ...
    if (programChanged) {
        ...
        gl.uniform1f(program.u_width, layer.paint['cubicline-width']);
    }

    painter.enableTileClippingMask(coord);

    //这里添加五个变量，分别是0 1  2  3  4的路况 hack me
    gl.uniform4f(program.u_speed_mat, curEleObj.getCalculatedSpeed(1), curEleObj.getCalculatedSpeed(2), curEleObj.getCalculatedSpeed(3), curEleObj.getCalculatedSpeed(4));

    const posMatrix = painter.translatePosMatrix(coord.posMatrix, tile, layer.paint['cubicline-translate'], layer.paint['cubicline-translate-anchor']);
