...
function draw(painter, source, layer, coords) {
    if (layer.paint['histogram-opacity'] === 0) return;
    const gl = painter.gl;
...
ExtrusionTexture.prototype.renderToMap = function() {
    ...
    gl.uniform1f(program.u_opacity, this.layer.paint['histogram-opacity']);
...
function drawExtrusion(painter, source, layer, coord) {
    ...
    const image = layer.paint['histogram-pattern'];
    const colorRender = layer.layout['histogram-color-render'];

    const layerData = buffers.layerData[layer.id];
    const programConfiguration = layerData.programConfiguration;
    const program = painter.useProgram(image ? 'histogramPattern' : (colorRender ? 'histogramColor' : 'histogram'), programConfiguration);
    ...
    if (colorRender) {
        const colors = layer.paint['histogram-colors'];
        const maxHeight = layer.paint['histogram-max-height'];

        let parsedColor = [];
        for (let i = 0; i < colors.length; i++) {
            let tmp = colorParser.parseCSSColor(colors[i]);
            tmp[0] /= 255.0;
            tmp[1] /= 255.0;
            tmp[2] /= 255.0;
            parsedColor.push(tmp);
        }
        gl.uniform1f(program.u_max_height, maxHeight*3);
        gl.uniform4fv(program.u_color_a, parsedColor[0]);
        gl.uniform4fv(program.u_color_b, parsedColor[1]);
        gl.uniform4fv(program.u_color_c, parsedColor[2]);
        gl.uniform4fv(program.u_color_d, parsedColor[3]);
        gl.uniform4fv(program.u_color_e, parsedColor[4]);
    }

    painter.gl.uniformMatrix4fv(program.u_matrix, false, painter.translatePosMatrix(
        coord.posMatrix,
        tile,
        layer.paint['histogram-translate'],
        layer.paint['histogram-translate-anchor']
    ));
