...
module.exports = function drawSprite(painter, sourceCache, layer, coords) {
    ...
    if (layer.paint['sprite-width'] <= 0) return;

    //这里处理关于速度的问题
    var speedFactor = parseInt(layer.paint['sprite-speed-factor']);
    ...
    const programId =
        layer.paint['sprite-dasharray'] ? 'lineSDF' :
        layer.paint['sprite-pattern'] ? 'linePattern' : 'sprite';
    ...
    for (const coord of coords) {
        ...
        drawSpriteTile(program, painter, tile, bucket.buffers, layer, coord, layerData, programChanged, tileRatioChanged);
        prevTileZoom = tile.coord.z;
        firstTile = false;
    }
};

function drawSpriteTile(program, painter, tile, buffers, layer, coord, layerData, programChanged, tileRatioChanged) {
    const gl = painter.gl;
    const dasharray = layer.paint['sprite-dasharray'];
    const image = layer.paint['sprite-pattern'];
    ...
    if (programChanged || tileRatioChanged) {
        ...
        if (dasharray) {
            posA = painter.lineAtlas.getDash(dasharray.from, layer.layout['sprite-cap'] === 'round');
            posB = painter.lineAtlas.getDash(dasharray.to, layer.layout['sprite-cap'] === 'round');
    ...
    if (programChanged) {
        ...
        gl.uniform1f(program.u_width, layer.paint['sprite-width']);
    }
    ...
    const posMatrix = painter.translatePosMatrix(coord.posMatrix, tile, layer.paint['sprite-translate'], layer.paint['sprite-translate-anchor']);
