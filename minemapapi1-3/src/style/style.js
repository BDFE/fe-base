...
const minemap = require('../util/minemap');
...
const MinemapGLFunction = require('../style-spec/function');
...
class Style extends Evented {
    ...
    constructor(stylesheet, map, options) {
        ...
        options = util.extend({
            validate: typeof stylesheet === 'string' ? !minemap.isMinemapURL(stylesheet) : true
        }, options);
        ...
        if (typeof stylesheet === 'string') {
            ajax.getJSON(minemap.normalizeStyleURL(stylesheet), stylesheetLoaded);
        } else {
    ...
    _updateWorkerLayers(updatedIds, removedIds) {
        const symbolOrder = this._updatedSymbolOrder ? this._order.filter((id) => this._layers[id].type === 'symbol' || this._layers[id].type === 'symtracking') : null;
     ...
     *
     * May throw an Error ('Unimplemented: METHOD') if the minemap-gl-style-spec
    ...
    addLayer(layerObject, before, options) {
        ...
        if (this._removedLayers[id] && layer.source) {
            ...
            // layer, causing, e.g.:
            // https://github.com/minemap/minemap-gl-js/issues/3633
        ...
        if (layer.type === 'symbol' || layer.type === 'symtracking') {
    ...
    setPaintProperty(layerId, name, value, klass) {
        ...
        const isFeatureConstant = !(
            value &&
            MinemapGLFunction.isFunctionDefinition(value) &&
            value.property !== '$zoom' &&
