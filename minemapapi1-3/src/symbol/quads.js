 ...
 * @private
 */
function SymbolQuad(anchorPoint, tl, tr, bl, br, tex, anchorAngle, glyphAngle, minScale, maxScale, seq, writingMode) {
...
function getIconQuads(anchor, shapedIcon, boxScale, line, layer, alongLine, shapedText, globalProperties, featureProperties) {
    ...
    return [new SymbolQuad(new Point(anchor.x, anchor.y), tl, tr, bl, br, shapedIcon.image.rect, 0, 0, minScale, Infinity, 1)];
}
...
function getGlyphQuads(anchor, shaping, boxScale, line, layer, alongLine) {
    ...
    for (let k = 0; k < positionedGlyphs.length; k++) {
        ...
        for (let i = 0; i < glyphInstances.length; i++) {
            ...
            quads.push(new SymbolQuad(instance.anchorPoint, tl, tr, bl, br, rect, anchorAngle, glyphAngle, glyphMinScale, instance.maxScale, 1, shaping.writingMode));
        }
    }
