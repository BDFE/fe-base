        ...
        for (const id in styleLayers) {
            ...
            if (styleLayer.type === 'line' || styleLayer.type === 'airline' || styleLayer.type === 'sprite' || styleLayer.type === 'tracking') {
                ...
                const width = getLineWidth(this.getPaintValue(`${typePrefix}-width`, styleLayer),
                    this.getPaintValue(`${typePrefix}-gap-width`, styleLayer));
                const offset = this.getPaintValue(`${typePrefix}-offset`, styleLayer);
                const translate = this.getPaintValue(`${typePrefix}-translate`, styleLayer);
            ...
            } else if (styleLayer.type === 'extrusion' || styleLayer.type === 'histogram') {
                let layerType = styleLayer.type;
                styleLayerDistance = translateDistance(this.getPaintValue(`${layerType}-translate`, styleLayer));
    ...
    filterMatching(result, matching, array, queryGeometry, filter, filterLayerIDs, styleLayers, bearing, pixelsToTileUnits) {
        ...
        for (let k = 0; k < matching.length; k++) {
            ...
            for (let l = 0; l < layerIDs.length; l++) {
                ...
                let translatedPolygon;//hack me
                if (styleLayer.type !== 'symbol' && styleLayer.type != 'symtracking') {
                    ...
                    if (styleLayer.type === 'line' || styleLayer.type === 'airline' || styleLayer.type === 'sprite' || styleLayer.type === 'tracking') {
                        ...
                        const halfWidth = pixelsToTileUnits / 2 * getLineWidth(
                            this.getPaintValue(`${typePrefix}-width`, styleLayer, feature),
                            this.getPaintValue(`${typePrefix}-gap-width`, styleLayer, feature));
                        const lineOffset = this.getPaintValue(`${typePrefix}-offset`, styleLayer, feature);
                        if (lineOffset) {
                            geometry = offsetLine(geometry, lineOffset * pixelsToTileUnits);
                        }
                        if (!multiPolygonIntersectsBufferedMultiLine(translatedPolygon, geometry, halfWidth)) continue;

                    } else if (styleLayer.type === 'fill' || styleLayer.type === 'extrusion' || styleLayer.type === 'histogram') {
                        const typePrefix = styleLayer.type;
                        translatedPolygon = translate(queryGeometry,
                            this.getPaintValue(`${typePrefix}-translate`, styleLayer, feature),
                            this.getPaintValue(`${typePrefix}-translate-anchor`, styleLayer, feature),
