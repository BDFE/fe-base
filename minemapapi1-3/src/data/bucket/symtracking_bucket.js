...
function addVertex(array, x, y, ox, oy, tx, ty, minzoom, maxzoom, labelminzoom, labelangle, seq) {
    array.emplaceBack(
            // a_pos_offset
            x,
            y,
...
class SymtrackingBucket {
        ...
        if (options.arrays) {
            ...
            for (const id in options.arrays) {
                if (options.arrays[id]) {
                    this.buffers[id] = new BufferGroup(symtrackingInterfaces[id], options.layers, options.zoom, options.arrays[id]);
                }
            }
        }
    }
    ...
    createArrays() {
        this.arrays = util.mapObject(symtrackingInterfaces, (programInterface) => {
    ...
    prepare(stacks, icons) {
        ...
        for (const feature of this.features) {
            //hack me
            this._seq = feature.properties.link_seq;
            ...
            if (shapedTextOrientations[WritingMode.horizontal] || shapedIcon) {
                this.addFeature(feature, shapedTextOrientations, shapedIcon, this._seq);
            }
        }
    }

    addFeature(feature, shapedTextOrientations, shapedIcon, seq) {
        const layout = this.layers[0].layout,
            glyphSize = 24,
        ...
        const addSymbolInstance = (line, anchor) => {
            ...
            this.addSymbolInstance(anchor, line, shapedTextOrientations, shapedIcon, this.layers[0],
                ...
                iconBoxScale, iconPadding, iconAlongLine, {zoom: this.zoom}, feature.properties, seq);
        };

        if (symbolPlacement === 'line') {
            for (const line of clipLine(feature.geometry, 0, 0, EXTENT, EXTENT, true)) {
                //hack me
            ...
            // https://github.com/minemap/minemap-js/issues/3808
        ...
        for (const symbolInstance of this.symbolInstances) {
            ...
            if (hasText) {
                ...
                if (glyphScale <= maxScale) {
                    this.addSymbols(this.arrays.glyph, symbolInstance.glyphQuads, glyphScale, layout['text-keep-upright'], textAlongLine, collisionTile.angle, symbolInstance.featureProperties, symbolInstance.seq,symbolInstance.writingModes);
                }
            }
            ...
            if (hasIcon) {
                ...
                if (iconScale <= maxScale) {
                    this.addSymbols(this.arrays.icon, symbolInstance.iconQuads, iconScale, layout['icon-keep-upright'], iconAlongLine, collisionTile.angle, symbolInstance.featureProperties,symbolInstance.seq);
                }
            }
    ...
    addSymbols(arrays, quads, scale, keepUpright, alongLine, placementAngle, featureProperties, seq, writingModes) {
        ...
        for (const symbol of quads) {
            ...
            addVertex(layoutVertexArray, anchorPoint.x, anchorPoint.y, tl.x, tl.y, tex.x, tex.y, minZoom, maxZoom, placementZoom, glyphAngle, seq);
            addVertex(layoutVertexArray, anchorPoint.x, anchorPoint.y, tr.x, tr.y, tex.x + tex.w, tex.y, minZoom, maxZoom, placementZoom, glyphAngle, seq);
            addVertex(layoutVertexArray, anchorPoint.x, anchorPoint.y, bl.x, bl.y, tex.x, tex.y + tex.h, minZoom, maxZoom, placementZoom, glyphAngle, seq);
            addVertex(layoutVertexArray, anchorPoint.x, anchorPoint.y, br.x, br.y, tex.x + tex.w, tex.y + tex.h, minZoom, maxZoom, placementZoom, glyphAngle, seq);
    ...
    addSymbolInstance(anchor, line, shapedTextOrientations, shapedIcon, layer, addToBuffers, collisionBoxArray, featureIndex, sourceLayerIndex, bucketIndex,
        textBoxScale, textPadding, textAlongLine,
        iconBoxScale, iconPadding, iconAlongLine, globalProperties, featureProperties, seq) {
        ...
        if (textBoxEndIndex > SymtrackingBucket.MAX_INSTANCES) util.warnOnce("Too many symbols being rendered in a tile.");
        if (iconBoxEndIndex > SymtrackingBucket.MAX_INSTANCES) util.warnOnce("Too many glyphs being rendered in a tile.");
...
SymtrackingBucket.programInterfaces = symtrackingInterfaces;

// this constant is based on the size of StructArray indexes used in a symbol
// bucket--namely, iconBoxEndIndex and textBoxEndIndex
// eg the max valid UInt16 is 65,535
SymtrackingBucket.MAX_INSTANCES = 65535;

module.exports = SymtrackingBucket;
