...
const airlineInterface = {
    ...
    paintAttributes: [
        {property: 'airline-color', type: 'Uint8'},
        {property: 'airline-blur', multiplier: 10, type: 'Uint8'},
        {property: 'airline-opacity', multiplier: 10, type: 'Uint8'},
        {property: 'airline-gap-width', multiplier: 10, type: 'Uint8', name: 'a_gapwidth'},
        {property: 'airline-offset', multiplier: 1, type: 'Int8'},
    ],
 ...
 * @private
 */
class AirlineBucket extends Bucket {
    constructor(options) {
        super(options, airlineInterface);
    }

    addFeature(feature) {
        const layout = this.layers[0].layout;
        const join = layout['airline-join'];
        const cap = layout['airline-cap'];
        const miterLimit = layout['airline-miter-limit'];
        const roundLimit = layout['airline-round-limit'];
        //hack me
        h_seq = feature.properties.link_seq;
        seq = h_seq;
    ...
    addLine(vertices, feature, join, cap, miterLimit, roundLimit) {
        ...
        for (let i = 0; i < len; i++) {
            //hack me
            h_seq = i === 0 ? feature.properties.start_height : feature.properties.end_height;
...
AirlineBucket.programInterface = airlineInterface;
