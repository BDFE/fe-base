...
const cubiclineInterface = {
    ...
    paintAttributes: [
        {property: 'cubicline-color', type: 'Uint8'},
        {property: 'cubicline-blur', multiplier: 10, type: 'Uint8'},
        {property: 'cubicline-opacity', multiplier: 10, type: 'Uint8'},
        {property: 'cubicline-gap-width', multiplier: 10, type: 'Uint8', name: 'a_gapwidth'},
        {property: 'cubicline-offset', multiplier: 1, type: 'Int8'},
    ],
    elementArrayType: createElementArrayType()
};
let curOpacity = 0, curNum = 0, curStatus = 1;
function addLineVertex(layoutVertexBuffer, point, extrude, tx, ty, dir, linesofar) {
    layoutVertexBuffer.emplaceBack(
        ...
        (linesofar * LINE_DISTANCE_SCALE) >> 6, curOpacity & 0xFF, parseInt(curNum), parseInt(curStatus));
}

/**
 * @private
 */
class CubiclineBucket extends Bucket {
    constructor(options) {
        super(options, cubiclineInterface);
    }

    addFeature(feature) {
        const layout = this.layers[0].layout;
        const join = layout['cubicline-join'];
        const cap = layout['cubicline-cap'];
        const miterLimit = layout['cubicline-miter-limit'];
        const roundLimit = layout['cubicline-round-limit'];

        curNum = feature.properties.link_seq ? feature.properties.link_seq : curNum;
        curNum %= 10;
...
CubiclineBucket.programInterface = cubiclineInterface;

module.exports = CubiclineBucket;
