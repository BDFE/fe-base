...
const trackingInterface = {
    ...
    paintAttributes: [
        {property: 'tracking-color', type: 'Uint8'},
        {property: 'tracking-blur', multiplier: 10, type: 'Uint8'},
        {property: 'tracking-opacity', multiplier: 10, type: 'Uint8'},
        {property: 'tracking-gap-width', multiplier: 10, type: 'Uint8', name: 'a_gapwidth'},
        {property: 'tracking-offset', multiplier: 1, type: 'Int8'},
    ],
...
function addLineVertex(layoutVertexBuffer, point, extrude, tx, ty, dir, linesofar) {
    layoutVertexBuffer.emplaceBack(
        ...
        (linesofar * LINE_DISTANCE_SCALE) >> 6, seq);
}

/**
 * @private
 */
class TrackingBucket extends Bucket {
    constructor(options) {
        super(options, trackingInterface);
    }

    addFeature(feature) {
        const layout = this.layers[0].layout;
        const join = layout['tracking-join'];
        const cap = layout['tracking-cap'];
        const miterLimit = layout['tracking-miter-limit'];
        const roundLimit = layout['tracking-round-limit'];
        //hack me
        seq = feature.properties.link_seq;
        for (const tracking of loadGeometry(feature, LINE_DISTANCE_BUFFER_BITS)) {
            this.addLine(tracking, feature, join, cap, miterLimit, roundLimit);
        }
    }
...
TrackingBucket.programInterface = trackingInterface;

module.exports = TrackingBucket;
