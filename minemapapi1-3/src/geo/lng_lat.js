 ...
 *
 * minemap uses longitude, latitude coordinate order (as opposed to latitude, longitude) to match GeoJSON.
 *
 * Note that any minemap method that accepts a `LngLat` object as an argument or option
 ...
 * @example
 * var ll = new minemap.LngLat(-73.9749, 40.7736);
 * @see [Get coordinates of the mouse pointer](https://www.minedata.cn/minedata-js/example/mouse-position/)
 * @see [Display a popup](https://www.minedata.cn/minedata-js/example/popup/)
 * @see [Highlight features within a bounding box](https://www.minedata.cn/minedata-js/example/using-box-queryrenderedfeatures/)
 * @see [Create a timeline animation](https://www.minedata.cn/minedata-js/example/timeline-animation/)
 */
class LngLat {
    constructor(lng, lat) {
     ...
     * @example
     * var ll = new minemap.LngLat(286.0251, 40.7736);
     * var wrapped = ll.wrap();
     * wrapped.lng; // = -73.9749
     */
    wrap() {
     ...
     * @example
     * var ll = new minemap.LngLat(170, 0);
     * var mapCenter = new minemap.LngLat(-170, 0);
 ...
 * var ll = minemap.LngLat.convert(arr);
