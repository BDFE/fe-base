...
/**
 * An `AttributionControl` control presents the map's [attribution information](https://www.minedata.cn/help/attribution/).
 *
 * @implements {IControl}
 ...
 * @example
 * var map = new minemap.Map({attributionControl: false})
 *     .addControl(new minemap.AttributionControl({
 *         compact: true
 *     }));
 */
class AttributionControl {
    ...
    onAdd(map) {
        ...
        this._map = map;
        this._container = DOM.create('div', 'minemap-ctrl minemap-ctrl-attrib');
    ...
    _updateEditLink() {
        if (!this._editLink) this._editLink = this._container.querySelector('.minemap-improve-map');
        if (this._editLink) {
            const center = this._map.getCenter();
            this._editLink.href = `https://www.minedata.cn/map-feedback/#/${
