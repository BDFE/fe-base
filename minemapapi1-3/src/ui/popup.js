 ...
 *  };
 * var popup = new minemap.Popup({offset:popupOffsets})
 *   .setLngLat(e.lngLat)
 *   .setHTML("<h1>Hello World!</h1>")
 *   .addTo(map);
 * @see [Display a popup](https://www.minedata.cn/minemap-js/example/popup/)
 * @see [Display a popup on hover](https://www.minedata.cn/minemap-js/example/popup-on-hover/)
 * @see [Display a popup on click](https://www.minedata.cn/minemap-js/example/popup-on-click/)
 */
class Popup extends Evented {
     ...
     *
     * @param {Map} map The minemap js map to add the popup to.
     * @returns {Popup} `this`
     */
    addTo(map) {
        this._map = map;
    ...
    _createContent() {
        ...
        this._content = DOM.create('div', 'minemap-popup-content', this._container);

        if (this.options.closeButton) {
            this._closeButton = DOM.create('button', 'minemap-popup-close-button', this._content);
    ...
    _update() {
        ...
        if (!this._container) {
            this._container = DOM.create('div', 'minemap-popup', this._map.getContainer());
            this._tip       = DOM.create('div', 'minemap-popup-tip', this._container);
        ...
        for (const key in anchorTranslate) {
            classList.remove(`minemap-popup-anchor-${key}`);
        }
        classList.add(`minemap-popup-anchor-${anchor}`);
